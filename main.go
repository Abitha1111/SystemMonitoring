...
type SystemInfoWrapper struct {
	System1Info SystemInfo `json:"Pragathi_Info"`
}
	...
	for _, line := range lines {
		if strings.Contains(line, ":22") && strings.Contains(line, "ESTAB") {
			...
			if len(parts) >= 5 {
				...
				if ip != "127.0.0.1" && ip != "192.168.54.226" {
...
func main() {
	...
	wsURL := "ws://localhost:8080/SystemMonitoring/serverws" // Replace with your actual WebSocket server URL
