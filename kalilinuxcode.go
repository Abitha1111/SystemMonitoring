Gmail	Kirthiga Balan <kirthigabalan1799@gmail.com>
go code kali
Kirthi Balan <kirthibalan99@gmail.com>	Fri, Oct 25, 2024 at 10:22 AM
To: Kirthiga Balan <kirthigabalan1799@gmail.com>
package main
...
type SystemInfoWrapper struct {
 System1Info SystemInfo `json:"Kirthiga"`
}
...
func gethardwareModel() (string, error) {
 ...
 osdetail := string(out)
 if strings.Contains(osdetail, "Hardware Model") {
 lines := strings.Split(osdetail, "\n")
 for _, line := range lines {
 if strings.Contains(line, "Hardware Model") {
 ...
 return "No Hardware Model info available", nil
}
...
func getVendorType() (string, error) {
 out, err := exec.Command("sudo", "hostnamectl").Output()
 if err != nil {
 return "", fmt.Errorf("Vendor information command not found")
 }

 osdetail := string(out)
 if strings.Contains(osdetail, "Hardware Vendor") {
 lines := strings.Split(osdetail, "\n")
 for _, line := range lines {
 if strings.Contains(line, "Hardware Vendor") {
 parts := strings.Split(line, ":")
 //fmt.Printf(parts[0])
 //fmt.Printf(parts[1])
 if len(parts) > 1 {
...
func getWiFiInfo() (string, error) {
 ...
 fields := strings.Fields(line)
 if len(fields) >= 3 {
 // Check if the device type is 'wifi'
 if fields[1] == "wifi" || fields[1] == "ethernet" {
 // Format the output as "<interface> : <type>"
 wifiInfo = fmt.Sprintf("%s : %s", fields[0], fields[1])
 break // Stop once we find the WiFi device
 }
 }
 }
 ...
 return "", fmt.Errorf("No WiFi or Ethernet connections available")
}
...
func getSSHInfo() (string, error) {
 ...
 if ip != "127.0.0.1" && ip != "192.168.11.228" {
...
func main() {
 ...
 wsURL := "ws://192.168.11.228:8080/SystemMonitoring/serverws" // Replace with your actual WebSocket server URL
