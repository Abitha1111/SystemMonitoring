...
type SystemInfoWrapper struct {
	System1Info SystemInfo `json:"Pragathi"`
}
...
func getWiFiInfo() (string, error) {
	// Run `nmcli -t -f TYPE,DEVICE,NAME connection show --active` to get active connections
	cmd := exec.Command("nmcli", "-t", "-f", "TYPE,DEVICE,NAME", "connection", "show", "--active")
	var out bytes.Buffer
	cmd.Stdout = &out
	err := cmd.Run()
	if err != nil {
	...
	fmt.Println("Debug nmcli output:", networkInfo) // Debug print
	...
	if len(lines) == 0 || (len(lines) == 1 && lines[0] == "") {
		return `"wifi":"No active connection found"`, nil
	}

	// Process each line to extract TYPE, DEVICE, and NAME
	for _, line := range lines {
		...
		if len(fields) >= 3 {
			...
			// Only return the first valid active connection
			if connectionType != "loopback" {
				return fmt.Sprintf("%s:%s:%s", connectionType, device, name), nil
			}
		}
	}

	// If no valid connection is found, return this message
	return `"wifi":"No active connection found"`, nil
}
...
func main() {
	...
	wsURL := "ws://192.168.11.240:8080/SystemMonitoring/serverws" // Replace with your actual WebSocket server URL
