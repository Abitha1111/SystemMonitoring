  ...
  <div v-else>
    <div class="grid">
      <div class="card" v-for="(systemInfo, key) in filteredSystemData" :key="key">
        ...
        <div class="metric">
          <img src="../assets/os .png" class="os" />
          <span class="label">OperatingSystem:</span>
        ...
        <div class="card-body">
          ...
          <div class="metric ssh-status">
            ...
            <span
              :class="{'ssh-active': isSshActive(systemInfo.ssh_info), 'ssh-inactive': isSshInactive(systemInfo.ssh_info)}"
            >{{ systemInfo.ssh_info }}</span>
          </div>
          <div class="metric">
            <i class="fa-solid fa-xmarks-lines"></i>
            <span class="label">Firewall Status:</span>
            <span
              :class="{'fwall-active': isFwActive(systemInfo.Firewallstatus), 'fwall-inactive': isFwInactive(systemInfo.Firewallstatus)}"
            >{{ systemInfo.Firewallstatus }}</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  setup() {
    ...
    const THRESHOLD = 15000; // Set the threshold to 15 seconds (15,000 ms)
    ...
    const batteryPercentage = batteryInfo => {
      ...
      return percentageMatch
        ? Math.min(100, parseInt(percentageMatch[1], 10))
        : 0;
    };

    const batteryTimeToEmpty = batteryInfo => {
      const timeMatch = batteryInfo.match(
        /time to (full|empty):\s*([\d.]+ hours|[\d.]+ minutes)/
      );
    ...
    const isSshActive = sshInfo => {
      return (
        sshInfo && sshInfo.toLowerCase().includes("active ssh connections")
      );
    };

    const isSshInactive = sshInfo => {
      return (
        sshInfo && sshInfo.toLowerCase().includes("no active ssh connections")
      );
    };

    const isFwActive = sshInfo => {
      return sshInfo && sshInfo.toLowerCase().includes("active");
    };

    const isFwInactive = sshInfo => {
      return sshInfo && sshInfo.toLowerCase().includes("inactive");
    };
    ...
    const fetchData = () => {
      try {
        socket = new WebSocket("ws://192.168.11.251:8080/SystemMonitoring/clientws");
